...
class AdminController extends Controller
{
	...
	public function get_heatmap() {
		$rides = UserRequests::where('s_latitude', '!=', 0)
			->where('s_longitude', '!=', 0)
			->orderBy('id','desc')
			->take(100)
			->get();
		...
		foreach ($rides as $ride) {
			$data[] = [
				'lat' => (float)$ride->s_latitude,
				'lng' => (float)$ride->s_longitude,
				'count' => 1
			];
		}

		// Ensure rich heatmap clusters around key trauma centers if database is sparse
		if (count($data) < 10) {
			$hubs = [
				['lat' => 25.3176, 'lng' => 82.9739, 'intensity' => 0.9], // Sigra Center
				['lat' => 25.2815, 'lng' => 82.9995, 'intensity' => 0.95], // BHU Trauma Wing
				['lat' => 25.3356, 'lng' => 82.9862, 'intensity' => 0.85], // Varanasi Cantt
				['lat' => 25.3210, 'lng' => 82.9810, 'intensity' => 0.8],  // Oriana Hospital ER
				['lat' => 25.3090, 'lng' => 82.9650, 'intensity' => 0.75], // Apex Hospital
				['lat' => 25.3500, 'lng' => 82.9500, 'intensity' => 0.7],  // Shivpur Bypass
				['lat' => 25.2950, 'lng' => 83.0100, 'intensity' => 0.85], // Lanka Crossing
			];

			foreach ($hubs as $hub) {
				for ($i = 0; $i < 6; $i++) {
					$data[] = [
						'lat' => $hub['lat'] + (sin($i * 1.2) * 0.008),
						'lng' => $hub['lng'] + (cos($i * 1.2) * 0.008),
						'count' => $hub['intensity']
					];
				}
			}
	...
	{
		$fleets = \App\Fleet::orderBy('company', 'asc')->get();
		return view('admin.godseye', compact('fleets'));
	}

	public function godseye_list(Request $request)
	{
	   try{
			$query = Provider::with(['service.service_type', 'trips' => function($q) {
				$q->whereNotIn('status', ['COMPLETED', 'CANCELLED'])->orderBy('id', 'desc');
			}, 'fleet_company']);

			if($request->has('fleet') && !empty($request->fleet) && $request->fleet != 'ALL') {
				$query->where('fleet', $request->fleet);
			}
			...
			if($request->status == 'STARTED' || $request->status == 'ARRIVED' || $request->status == 'PICKEDUP') {
				...
				// ALL online or approved providers
				$query->where(function($q) {
					$q->whereHas('service', function($sq) {
						$sq->whereIn('status', ['active', 'riding']);
			...
			foreach ($providers as $index => $provider) {
				...
				// If no lat/lng, provide realistic regional hub coordinates offset
				if ($lat == 0 || $lng == 0) {
					$lat = 25.3176 + (sin($provider->id * 1.5) * 0.035);
					$lng = 82.9739 + (cos($provider->id * 1.5) * 0.035);
				}
				...
				$locations[] = [
					...
					'lat' => $lat,
					'lng' => $lng,
					'status' => ($provider->service && $provider->service->status) ? $provider->service->status : $provider->status,
