$(document).ready(function(){
    ...
    /* Sidebar - on click */
	$('.site-sidebar li.with-sub > a').on('click', function(e) {
		e.preventDefault();
		e.stopPropagation();
		if (!$('body').hasClass('compact-sidebar')) {
			var $parent = $(this).parent('li.with-sub');
			if ($parent.hasClass('active')) {
				$parent.removeClass('active');
				$parent.find('>ul').slideUp(250);
			} else {
				if (!$parent.parent().closest('.with-sub').length) {
					$('.site-sidebar li.with-sub').not($parent).removeClass('active').find('>ul').slideUp(250);
				}
				$parent.addClass('active');
				$parent.find('>ul').slideDown(250);
			}
		}
		return false;
	});
